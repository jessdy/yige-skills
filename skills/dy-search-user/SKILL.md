---
name: dy-search-user
description: 按关键词搜索抖音用户，cursor 翻页。需要一格数据 API Key。触发词：抖音搜用户、找达人、dy-search-user
---

## 简介

抖音搜索用户 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

关键词搜索用户，拿到 sec_user_id 后再拉资料/主页作品。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/search/fetch_user_search_v2`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-user-profile`、`dy-user-posts`、`dy-search-suggest` 组成完整工作流
...
# 将 skills/dy-search-user 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --keyword 美妆博主` | 常用示例 |
...
|------|------|
| Method | `POST` |
| Path | `/v1/douyin/search/fetch_user_search_v2` |
...
### 注意事项
- 结果中的 sec_user_id 用于后续用户接口
- nickname 可能重名，以 sec_user_id 为准
- 按次计费，请关注控制台余额
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `POST /v1/douyin/search/fetch_user_search_v2` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-user-profile`、`dy-user-posts`、`dy-search-suggest`
