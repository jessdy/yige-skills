...
def main() -> None:
    p = argparse.ArgumentParser(description="公众号文章评论")
    add_common(p)
    p.add_argument("--url", required=True)
    p.add_argument("--buffer", default="", help="首页留空；翻页传上一页 buffer")
    args = p.parse_args()
    api_key = resolve_key(args.api_key)
    body = {"url": args.url, "raw": bool(args.raw)}
    if args.buffer:
        body["buffer"] = args.buffer
    result = post("/v1/wechat_mp/v2/fetch_article_comments", body, api_key)
