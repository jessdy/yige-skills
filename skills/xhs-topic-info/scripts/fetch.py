...
def main() -> None:
    p = argparse.ArgumentParser(description="小红书话题详情")
    add_common(p)
    p.add_argument("--page-id", required=True)
    p.add_argument("--note-id", default="")
    p.add_argument("--source", default="normal")
    args = p.parse_args()
    api_key = resolve_key(args.api_key)
    result = request("GET", PATH, api_key, params={"page_id": args.page_id, "source": args.source, "note_id": args.note_id or None})
