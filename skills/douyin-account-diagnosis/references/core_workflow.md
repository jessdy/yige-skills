...
1. 检查 API Key 可用性：
   - 命令行参数 `--api-key` > 环境变量 `YIGE_API_KEY` > 内置默认密钥
2. 内置默认密钥缺失且未配置 → 引导用户前往 [一格hub](https://yige.zone/settings/api-keys?source=github) 获取并配置
3. 鉴权就绪 → 进入 Step 1
...
2. 接口信息：
   - 接口地址：`POST https://yige.zone/story/api/dyUser/queryData`
...
8. **数据来源**：报告末尾注明 一格数据Yige API (yige.zone)
