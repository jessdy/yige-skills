---
name: dy-video-detail
description: 根据 aweme_id 获取单个抖音作品详情（文案、作者、点赞评论等）。播放量请另用 dy-video-stats。需要一格数据 API Key。触发词：抖音作品详情、抖音视频详情、dy-video-detail
---

## 简介

抖音作品详情 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/app/v3/fetch_one_video`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-search-video`、`dy-video-detail`、`dy-user-profile`、`dy-video-comments`、`dy-video-stats`、`dy-video-by-share` 组成完整工作流
...
# 将 skills/dy-video-detail 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --aweme-id 7123456789012345678` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/douyin/app/v3/fetch_one_video` |
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `aweme_id` | 是 | 作品 ID（aweme_id） |

### 注意事项
- 用户主键多为 sec_user_id，可从详情作者字段取得
- 播放量不在本接口，请用 dy-video-stats
- 按次计费，请关注控制台余额
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/douyin/app/v3/fetch_one_video` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-search-video`、`dy-video-detail`、`dy-user-profile`、`dy-video-comments`、`dy-video-stats`、`dy-video-by-share`
