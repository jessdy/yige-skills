---
name: xhs-topic-feed
description: 拉取话题下笔记列表，支持最热/最新与游标翻页。 需要一格数据 API Key。触发词：小红书话题笔记列表、小红书数据、xhs-topic-feed
---

## 简介

小红书话题笔记列表 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

拉取话题下笔记列表，支持最热/最新与游标翻页。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/app_v2/get_topic_feed`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-topic-feed 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --page-id 5c1cc866febed9000184b7c1 --sort trend` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/xiaohongshu/app_v2/get_topic_feed` |
...
| `last_note_id` | 否 | 见 --help |
| `last_note_ct` | 否 | 见 --help |
| `session_id` | 否 | 见 --help |
| `first_load_time` | 否 | 见 --help |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/xiaohongshu/app_v2/get_topic_feed` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/420136408e0
