...
description: 拉取蒲公英侧博主笔记表现明细。 需要一格数据 API Key。触发词：蒲公英博主笔记明细、小红书数据、xhs-pgy-blogger-notes
---

## 简介

蒲公英博主笔记明细 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/pgy/get_blogger_notes`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-pgy-blogger-notes 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --user-id 5c668b3e0000000012021605 --order-type 2` | 常用示例 |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `POST /v1/xiaohongshu/pgy/get_blogger_notes` |
