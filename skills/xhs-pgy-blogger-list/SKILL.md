...
description: 按品牌广告主 ID 与粉丝区间筛选蒲公英博主列表。 需要一格数据 API Key。触发词：蒲公英博主选号列表、小红书数据、xhs-pgy-blogger-list
---

## 简介

蒲公英博主选号列表 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

按品牌广告主 ID 与粉丝区间筛选蒲公英博主列表。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/pgy/get_blogger_list`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-pgy-blogger-list 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --brand-user-id 5dcfa5370000000001006030 --fans-number-lower 10000 --fans-number-upper 1000000` | 常用示例 |
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `brand_user_id` | 是 | 见 --help |
| `page_num` | 否 | 默认 1 |
| `page_size` | 否 | 默认 20 |
| `fans_number_lower` | 否 | 见 --help |
| `fans_number_upper` | 否 | 见 --help |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `POST /v1/xiaohongshu/pgy/get_blogger_list` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://yige.zone/docs
