...
description: 获取蒲公英博主资料、粉丝、合作报价等。 需要一格数据 API Key。触发词：蒲公英博主详情、小红书数据、xhs-pgy-blogger-detail
---

## 简介

蒲公英博主详情 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/pgy/get_blogger_detail`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-pgy-blogger-detail 放入你的 Agent Skills 目录
```
...
|------|------|
| Method | `POST` |
| Path | `/v1/xiaohongshu/pgy/get_blogger_detail` |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `POST /v1/xiaohongshu/pgy/get_blogger_detail` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/479321008e0
