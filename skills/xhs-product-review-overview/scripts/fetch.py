...
def main() -> None:
    p = argparse.ArgumentParser(description="小红书商品评价总览")
    add_common(p)
    p.add_argument("--sku-id", required=True)
    p.add_argument("--tab", type=int, default=2)
    args = p.parse_args()
    api_key = resolve_key(args.api_key)
    result = request("GET", PATH, api_key, params={"sku_id": args.sku_id, "tab": args.tab})
