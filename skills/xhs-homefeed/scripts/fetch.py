...
def main() -> None:
    p = argparse.ArgumentParser(description="小红书首页推荐")
    add_common(p)
    p.add_argument("--num", type=int, default=20)
    p.add_argument("--cursor-score", default="")
    p.add_argument("--category", default="")
    p.add_argument("--need-filter-image", default="false", choices=["true", "false"])
    args = p.parse_args()
    api_key = resolve_key(args.api_key)
    params = {
        "num": args.num,
        "cursor_score": args.cursor_score or None,
        "category": args.category or None,
        "need_filter_image": args.need_filter_image,
    }
