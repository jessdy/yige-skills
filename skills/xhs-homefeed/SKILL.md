---
name: xhs-homefeed
description: 拉取小红书首页推荐信息流，可按分类与图文筛选翻页。 需要一格数据 API Key。触发词：小红书首页推荐、小红书数据、xhs-homefeed
---

## 简介

小红书首页推荐 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

拉取小红书首页推荐信息流，可按分类与图文筛选翻页。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/web_v3/fetch_homefeed`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-homefeed 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --num 20` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/xiaohongshu/web_v3/fetch_homefeed` |
| Host | `https://yige.zone` |
| 鉴权 | `Authorization: Bearer <YIGE_API_KEY>` |
| 超时 | 建议 30s |
| 文档 | https://docs.tikhub.io/438852175e0 |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/xiaohongshu/web_v3/fetch_homefeed` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/438852175e0
