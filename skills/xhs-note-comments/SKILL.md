...
description: 拉取笔记一级评论列表，支持 cursor/index 翻页与最新/最热排序。 需要一格数据 API Key。触发词：小红书笔记评论、小红书数据、xhs-note-comments
---

## 简介

小红书笔记评论 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

拉取笔记一级评论列表，支持 cursor/index 翻页与最新/最热排序。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/app_v2/get_note_comments`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-note-comments 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --note-id 697c0eee000000000a03c308 --sort-strategy like_count` | 常用示例 |
...
| `cursor` | 否 | 见 --help |
| `index` | 否 | 默认 0 |
| `page_area` | 否 | 默认 "UNFOLDED" |
| `sort_strategy` | 否 | 默认 "latest_v2" |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/xiaohongshu/app_v2/get_note_comments` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/420136394e0
