---
name: dy-search-video
description: 按关键词搜索抖音视频，支持排序、时长、发布时间等筛选，翻页需透传 cursor/search_id/backtrace。需要一格数据 API Key。触发词：抖音搜索、搜索视频、dy-search-video
---

## 简介

抖音搜索视频 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/search/fetch_video_search_v2`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-search-video`、`dy-video-detail`、`dy-user-profile`、`dy-video-comments`、`dy-search-general`、`dy-search-suggest` 组成完整工作流
...
# 将 skills/dy-search-video 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --keyword 猫咪 --sort-type 1` | 常用示例 |
...
|------|------|
| Method | `POST` |
| Path | `/v1/douyin/search/fetch_video_search_v2` |
...
| `keyword` | 是 | 搜索关键词 |
| `cursor` | 否 | 翻页游标，默认 0 |
| `sort_type` | 否 | 0=综合 1=最多点赞 2=最新发布 |
| `publish_time` | 否 | 0不限 / 1一天 / 7一周 / 180半年 |
| `filter_duration` | 否 | 时长过滤：0 / 0-1 / 1-5 / 5-10000 |
| `content_type` | 否 | 0不限 1视频 2图片 3文章 |
| `search_id` | 否 | 翻页搜索 ID，透传上次返回 |
| `backtrace` | 否 | 翻页回溯标识，透传上次返回 |

### 注意事项
- 翻页必须透传 search_id / backtrace / cursor
- 需要播放量时再调 dy-video-stats
- 按次计费，请关注控制台余额
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `POST /v1/douyin/search/fetch_video_search_v2` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-search-video`、`dy-video-detail`、`dy-user-profile`、`dy-video-comments`、`dy-search-general`、`dy-search-suggest`
