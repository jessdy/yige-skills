#!/usr/bin/env python3
"""Call Yige Douyin API. Auth via YIGE_API_KEY env or --api-key."""
...
def main() -> None:
    p = argparse.ArgumentParser(description="抖音搜索视频")
    add_common(p)
    p.add_argument('--keyword', required=True)
    p.add_argument('--cursor', type=int, default=0)
    p.add_argument('--sort-type', dest='sort_type', default='0', help='0综合 1最多点赞 2最新')
    p.add_argument('--publish-time', dest='publish_time', default='0', help='0不限 1最近一天 7最近一周 180半年')
    p.add_argument('--filter-duration', dest='filter_duration', default='0', help='0不限 / 0-1 / 1-5 / 5-10000')
    p.add_argument('--content-type', dest='content_type', default='0', help='0不限 1视频 2图片 3文章')
    p.add_argument('--search-id', dest='search_id', default='')
    p.add_argument('--backtrace', default='')
    args = p.parse_args()
