...
description: Web V3 笔记详情；需 note_id 与分享链接中的 xsec_token。优先用 App V2 图文/视频详情。 需要一格数据 API Key。触发词：小红书笔记详情(Web)、小红书数据、xhs-web-note-detail
---

## 简介

小红书笔记详情(Web) 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

Web V3 笔记详情；需 note_id 与分享链接中的 xsec_token。优先用 App V2 图文/视频详情。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/web_v3/fetch_note_detail`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-web-note-detail 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --note-id xxx --xsec-token yyy` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/xiaohongshu/web_v3/fetch_note_detail` |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/xiaohongshu/web_v3/fetch_note_detail` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/438852168e0
