...
def main() -> None:
    p = argparse.ArgumentParser(description="蒲公英博主笔记转化率")
    add_common(p)
    p.add_argument("--user-id", required=True)
    p.add_argument("--note-type", type=int, default=3)
    p.add_argument("--date-type", type=int, default=1)
    p.add_argument("--advertise-switch", type=int, default=1)
    args = p.parse_args()
    api_key = resolve_key(args.api_key)
    result = request("POST", PATH, api_key, body={"user_id": args.user_id, "note_type": args.note_type, "date_type": args.date_type, "advertise_switch": args.advertise_switch})
