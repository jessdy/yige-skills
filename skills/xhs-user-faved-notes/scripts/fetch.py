...
def main() -> None:
    p = argparse.ArgumentParser(description="小红书用户收藏笔记")
    add_common(p)
    ...
    if not args.user_id and not args.share_text:
        p.error("需要 --user-id 或 --share-text")
    result = request("GET", PATH, api_key, params={"user_id": args.user_id or None, "share_text": args.share_text or None, "cursor": args.cursor or None})
