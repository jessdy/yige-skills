...
description: 拉取用户公开收藏的笔记列表，cursor 翻页。 需要一格数据 API Key。触发词：小红书用户收藏笔记、小红书数据、xhs-user-faved-notes
---

## 简介

小红书用户收藏笔记 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

拉取用户公开收藏的笔记列表，cursor 翻页。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/app_v2/get_user_faved_notes`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-user-faved-notes 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --user-id 5a8cf39111be10466d285d6b` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/xiaohongshu/app_v2/get_user_faved_notes` |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/xiaohongshu/app_v2/get_user_faved_notes` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/420136397e0
