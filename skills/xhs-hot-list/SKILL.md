---
name: xhs-hot-list
description: 获取小红书热榜数据。 需要一格数据 API Key。触发词：小红书热榜、小红书数据、xhs-hot-list
---

## 简介

小红书热榜 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/web_v3/fetch_hot_list`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-hot-list 放入你的 Agent Skills 目录
```
...
|------|------|
| Method | `GET` |
| Path | `/v1/xiaohongshu/web_v3/fetch_hot_list` |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/xiaohongshu/web_v3/fetch_hot_list` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://tikhub.io/zh/xiaohongshu-api
