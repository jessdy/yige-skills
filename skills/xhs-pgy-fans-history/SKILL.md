...
description: 博主粉丝增长历史趋势。 需要一格数据 API Key。触发词：蒲公英粉丝增长历史、小红书数据、xhs-pgy-fans-history
---

## 简介

蒲公英粉丝增长历史 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/pgy/get_blogger_fans_history`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-pgy-fans-history 放入你的 Agent Skills 目录
```
...
|------|------|
| Method | `POST` |
| Path | `/v1/xiaohongshu/pgy/get_blogger_fans_history` |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `POST /v1/xiaohongshu/pgy/get_blogger_fans_history` |
