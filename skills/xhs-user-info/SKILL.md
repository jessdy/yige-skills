---
name: xhs-user-info
description: 根据 user_id 或分享链接获取用户主页信息（昵称、简介、粉丝数等）。需要一格数据 API Key。
---

## 简介

调用[一格数据](https://yige.zone) 小红书接口 `GET /v1/xiaohongshu/app_v2/get_user_info`，返回结构化 JSON。

**何时使用：** 用户要看某个账号资料、粉丝数、简介；有 user_id 或主页/分享链接
...
| `user_id` | 否 | 用户 ID，优先 |
| `share_text` | 否 | 小红书用户分享链接（APP/Web） |

接口文档参考：https://docs.tikhub.io/420136395e0
