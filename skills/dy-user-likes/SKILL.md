---
name: dy-user-likes
description: 拉取用户公开喜欢（点赞）作品列表。需要一格数据 API Key。触发词：抖音喜欢列表、抖音点赞、dy-user-likes
---

## 简介

抖音用户喜欢作品 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/app/v3/fetch_user_like_videos`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-user-profile`、`dy-user-posts` 组成完整工作流
...
# 将 skills/dy-user-likes 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --sec-user-id MS4wLjABAAAA... --counts 20` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/douyin/app/v3/fetch_user_like_videos` |
...
| `max_cursor` | 否 | 翻页游标，默认 0 |
| `counts` | 否 | 每页数量，默认 20（注意字段名是 counts） |

### 注意事项
- 隐私设置可能导致无数据
- 参数名是 counts 不是 count
- 按次计费，请关注控制台余额
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/douyin/app/v3/fetch_user_like_videos` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-user-profile`、`dy-user-posts`
