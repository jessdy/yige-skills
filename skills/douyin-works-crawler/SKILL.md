...
- **📊 账号信息查询**：输入抖音昵称或抖音号，一键获取账号基础数据（粉丝数、获赞、作品总数、一格指数等）
...
2. 配置环境变量：
   ```bash
   # macOS/Linux
   export YIGE_API_KEY=你的API密钥值

   # Windows PowerShell
   $env:YIGE_API_KEY="你的API密钥值"
   ```
3. 运行查询：
   ```bash
...
| 变量名 | 必填 | 说明 |
|--------|------|------|
| `YIGE_API_KEY` | 是 | 一格数据API密钥（格式 `ak_xxx`） |
...
**Q1: 运行时提示"未设置环境变量 YIGE_API_KEY"怎么办？**
...
A: 前往一格数据平台注册并申请API密钥，格式为 `ak_xxx`。
...
A: 一格数据API按调用次数计费，请前往一格数据平台充值积分。
