...
- 其他维度百分制 = 内容核心数据(不含一格数据)x0.4 + 双维度诊断(健康度+活跃度平均)x0.35 + 运营规范性x0.25

| 评估模块 | 原始分 | 作用 | 对综合评分的影响 |
|---------|--------|------|----------------|
| **内容核心数据**（不含一格数据） | 0-31分 | 调整分输入 | 微调 |
...
*诊断时间：[YYYY-MM-DD] | 数据来源：一格数据API接口 | 置信度：[高/中] | 阅读数>10万显示为10w+ | 禁止输出西瓜指数*
...
### 数据准确性
- 📊 **接口数据：** 所有数据来自一格数据API接口，确保权威性和准确性
