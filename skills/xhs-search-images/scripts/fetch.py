...
def main() -> None:
    p = argparse.ArgumentParser(description="小红书搜索图片")
    add_common(p)
    p.add_argument("--keyword", required=True)
    p.add_argument("--page", type=int, default=1)
    p.add_argument("--search-id", default="")
    p.add_argument("--search-session-id", default="")
    p.add_argument("--word-request-id", default="")
    args = p.parse_args()
    api_key = resolve_key(args.api_key)
    result = request("GET", PATH, api_key, params={
        "keyword": args.keyword, "page": args.page, "source": "explore_feed",
        "search_id": args.search_id or None, "search_session_id": args.search_session_id or None,
        "word_request_id": args.word_request_id or None,
    })
