...
description: 按关键词搜索小红书图片结果。 需要一格数据 API Key。触发词：小红书搜索图片、小红书数据、xhs-search-images
---

## 简介

小红书搜索图片 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/app_v2/search_images`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-search-images 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --keyword 壁纸` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/xiaohongshu/app_v2/search_images` |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/xiaohongshu/app_v2/search_images` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/420136400e0
