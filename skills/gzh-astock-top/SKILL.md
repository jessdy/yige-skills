---
name: gzh-astock-top
description: A股公众号大V工具，输入日期获取A股领域公众号大V账号30个及账号数据（平均阅读、一格指数等），同时获取每个账号最新一篇文章及文章数据。当用户提到"A股公众号"、"股市大V"、"A股账号"、"股票公众号榜单"、"A股公众号大V"时使用。
dependency:
  python:
...
> API 请求均携带 `A股公众号大V-GitHub` 标识。使用环境变量 `YIGE_API_KEY` 鉴权。
...
- **账号数据**：获取每个账号的平均阅读数、一格指数、账号描述等核心指标
...
- **数据说明**：账号数据来源于一格数据API收录的公众号信息，文章数据为账号在**指定日期**发布的作品（由服务端直接筛选）。互动数据截止为入库时间，非实时数据。
- **排序说明**：账号按平均阅读数降序排列
...
| # | 公众号 | 平均阅读 | 一格指数 | 最新文章 | AI概要 | 阅读 | 点赞 | 发布 |
|---|-------|---------|---------|---------|--------|------|------|------|
...
> **数据更新时间**：一格数据平台每日 **07:00** 更新昨日爆款文章数据，建议 **07:30** 之后运行推送查询。
...
| 数据接口 | 一格数据 API (Yige) — dailyPublish 接口 | 通过 YIGE_API_KEY 鉴权，**1次调用**获取全部数据 |
...
**Q: 数据更新频率？**
A: 一格数据平台每日 **07:00** 更新前一日数据，建议 07:30 后查询昨日日报。
