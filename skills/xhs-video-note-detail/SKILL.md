---
name: xhs-video-note-detail
description: 根据 note_id 或分享链接获取视频笔记详情与互动数据。 需要一格数据 API Key。触发词：小红书视频笔记详情、小红书数据、xhs-video-note-detail
---

## 简介

小红书视频笔记详情 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

根据 note_id 或分享链接获取视频笔记详情与互动数据。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/app_v2/get_video_note_detail`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-video-note-detail 放入你的 Agent Skills 目录
```
...
```bash
export YIGE_API_KEY=ak_xxx
python scripts/fetch.py --share-text "https://www.xiaohongshu.com/explore/..."
```
...
| `python scripts/fetch.py --share-text "https://www.xiaohongshu.com/explore/..."` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/xiaohongshu/app_v2/get_video_note_detail` |
| Host | `https://yige.zone` |
| 鉴权 | `Authorization: Bearer <YIGE_API_KEY>` |
| 超时 | 建议 30s |
| 文档 | https://docs.tikhub.io/420136392e0 |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/xiaohongshu/app_v2/get_video_note_detail` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/420136392e0
