...
description: Web V3 用户主页信息。优先用 App V2 xhs-user-info。 需要一格数据 API Key。触发词：小红书用户信息(Web)、小红书数据、xhs-web-user-info
---

## 简介

小红书用户信息(Web) 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

Web V3 用户主页信息。优先用 App V2 xhs-user-info。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/web_v3/fetch_user_info`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-web-user-info 放入你的 Agent Skills 目录
```
...
|------|------|
| Method | `GET` |
| Path | `/v1/xiaohongshu/web_v3/fetch_user_info` |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/xiaohongshu/web_v3/fetch_user_info` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/438852177e0
