---
name: dy-user-fans
description: 拉取用户粉丝列表，支持 max_time 翻页。需要一格数据 API Key。触发词：抖音粉丝、dy-user-fans
---

## 简介

抖音用户粉丝列表 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/app/v3/fetch_user_fans_list`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-user-profile`、`dy-search-user` 组成完整工作流
...
# 将 skills/dy-user-fans 放入你的 Agent Skills 目录
```
...
|------|------|
| Method | `GET` |
| Path | `/v1/douyin/app/v3/fetch_user_fans_list` |
...
| `max_time` | 否 | 翻页时间戳，默认 0；下一页用上次返回值 |
| `count` | 否 | 数量，默认 20 |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/douyin/app/v3/fetch_user_fans_list` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-user-profile`、`dy-search-user`
