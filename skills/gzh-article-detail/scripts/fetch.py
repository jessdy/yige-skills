...
def main() -> None:
    p = argparse.ArgumentParser(description="公众号文章详情")
    add_common(p)
    p.add_argument("--url", required=True, help="https://mp.weixin.qq.com/s/…")
    args = p.parse_args()
    api_key = resolve_key(args.api_key)
    body = {"url": args.url, "raw": bool(args.raw)}
    result = post("/v1/wechat_mp/v2/fetch_article_detail", body, api_key)
