...
description: 根据文章 URL 拉取正文、标题、作者、封面、发布时间、合集信息等。需要一格数据 API Key。
---

## 简介

调用[一格数据](https://yige.zone) 公众号接口 `/v1/wechat_mp/v2/fetch_article_detail`，返回结构化 JSON。

**何时使用：** 用户给了 mp.weixin.qq.com 文章链接，要读正文、标题、作者或归档
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `url` | 是 | 文章链接 https://mp.weixin.qq.com/s/… |
| — | — | 无额外可选参数（可用 `--raw`） |
| `raw` | 否 | 默认 false（精简）；`--raw` 为 true |

接口文档参考：https://docs.tikhub.io/472974851e0
