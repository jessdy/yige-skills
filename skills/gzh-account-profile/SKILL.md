...
description: 根据公众号 gh_username（gh_…）查询公众号资料页：昵称、简介、头像、服务类型等。需要一格数据 API Key。
---

## 简介

调用[一格数据](https://yige.zone) 公众号接口 `/v1/wechat_mp/v2/fetch_account_profile`，返回结构化 JSON。

**何时使用：** 用户要查某个公众号的资料、简介、头像，或只有 gh_username 时
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `username` | 是 | 公众号 gh_username，形如 gh_363b924965e9 |
| — | — | 无额外可选参数（可用 `--raw`） |
| `raw` | 否 | 默认 false（精简）；`--raw` 为 true |

接口文档参考：https://docs.tikhub.io/472974857e0
