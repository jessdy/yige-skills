...
description: 博主粉丝量级、活跃/互动/阅读粉丝等概览。 需要一格数据 API Key。触发词：蒲公英粉丝概览、小红书数据、xhs-pgy-fans-summary
---

## 简介

蒲公英粉丝概览 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

博主粉丝量级、活跃/互动/阅读粉丝等概览。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/pgy/get_blogger_fans_summary`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-pgy-fans-summary 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --user-id 5c668b3e0000000012021605` | 常用示例 |
...
|------|------|
| Method | `POST` |
| Path | `/v1/xiaohongshu/pgy/get_blogger_fans_summary` |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `POST /v1/xiaohongshu/pgy/get_blogger_fans_summary` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/479321013e0
