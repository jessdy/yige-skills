---
name: dy-comment-replies
description: 拉取某条评论下的回复列表（楼中楼）。需要一格数据 API Key。触发词：抖音评论回复、楼中楼、dy-comment-replies
---

## 简介

抖音评论回复 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/app/v3/fetch_video_comment_replies`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-video-comments`、`dy-video-detail` 组成完整工作流
...
# 将 skills/dy-comment-replies 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --item-id 7123... --comment-id 7234...` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/douyin/app/v3/fetch_video_comment_replies` |
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `item_id` | 是 | 作品 ID（接口字段名 item_id） |
| `comment_id` | 是 | 一级评论 ID |
| `cursor` | 否 | 翻页游标，默认 0 |
| `count` | 否 | 数量，默认 20 |

### 注意事项
- 先 dy-video-comments 再取 comment_id
- item_id 即 aweme_id
- 按次计费，请关注控制台余额
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/douyin/app/v3/fetch_video_comment_replies` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-video-comments`、`dy-video-detail`
