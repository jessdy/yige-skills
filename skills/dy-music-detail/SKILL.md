---
name: dy-music-detail
description: 根据 music_id 获取音乐详情。需要一格数据 API Key。触发词：抖音音乐、BGM、dy-music-detail
---

## 简介

抖音音乐详情 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/app/v3/fetch_music_detail`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-music-videos`、`dy-video-detail` 组成完整工作流
...
# 将 skills/dy-music-detail 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --music-id 123456789` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/douyin/app/v3/fetch_music_detail` |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/douyin/app/v3/fetch_music_detail` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-music-videos`、`dy-video-detail`
