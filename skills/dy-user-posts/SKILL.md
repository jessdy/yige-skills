---
name: dy-user-posts
description: 拉取用户主页公开作品列表，支持 max_cursor 翻页。需要一格数据 API Key。触发词：抖音主页作品、抖音用户视频、dy-user-posts
---

## 简介

抖音用户主页作品 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

按 sec_user_id 拉取用户主页作品列表，用 max_cursor 翻页，适合达人内容盘点与复盘。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/app/v3/fetch_user_post_videos`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-user-profile`、`dy-video-detail`、`dy-video-stats` 组成完整工作流
...
# 将 skills/dy-user-posts 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --sec-user-id MS4wLjABAAAA... --count 20` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/douyin/app/v3/fetch_user_post_videos` |
...
| `max_cursor` | 否 | 翻页游标，默认 0；下一页用上次返回值 |
| `count` | 否 | 每页数量，默认 20 |
| `sort_type` | 否 | 排序类型，默认 0 |

### 注意事项
- 翻页透传 max_cursor
- 需要播放量时对返回 ID 再调 dy-video-stats
- 按次计费，请关注控制台余额
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/douyin/app/v3/fetch_user_post_videos` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-user-profile`、`dy-video-detail`、`dy-video-stats`
