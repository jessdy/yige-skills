...
def main() -> None:
    p = argparse.ArgumentParser(description="抖音用户主页作品")
    add_common(p)
    p.add_argument('--sec-user-id', required=True, dest='sec_user_id')
    p.add_argument('--max-cursor', dest='max_cursor', type=int, default=0)
    p.add_argument('--count', type=int, default=20)
    p.add_argument('--sort-type', dest='sort_type', type=int, default=0, help='排序类型，默认 0')
    args = p.parse_args()
