...
def main() -> None:
    p = argparse.ArgumentParser(description="小红书搜索群聊")
    add_common(p)
    p.add_argument("--keyword", required=True)
    p.add_argument("--page-no", type=int, default=0)
    p.add_argument("--search-id", default="")
    p.add_argument("--is-recommend", type=int, default=0)
    args = p.parse_args()
    api_key = resolve_key(args.api_key)
    result = request("GET", PATH, api_key, params={"keyword": args.keyword, "page_no": args.page_no, "search_id": args.search_id or None, "source": "unifiedSearchGroup", "is_recommend": args.is_recommend})
