...
def main() -> None:
    p = argparse.ArgumentParser(description="小红书商品评价列表")
    add_common(p)
    p.add_argument("--sku-id", required=True)
    p.add_argument("--page", type=int, default=0)
    p.add_argument("--sort-strategy-type", type=int, default=0)
    p.add_argument("--share-pics-only", type=int, default=0)
    args = p.parse_args()
    api_key = resolve_key(args.api_key)
    result = request("GET", PATH, api_key, params={"sku_id": args.sku_id, "page": args.page, "sort_strategy_type": args.sort_strategy_type, "share_pics_only": args.share_pics_only, "from_page": "score_page"})
