...
description: 拉取商品评价列表，支持排序与仅看有图。 需要一格数据 API Key。触发词：小红书商品评价列表、小红书数据、xhs-product-reviews
---

## 简介

小红书商品评价列表 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/app_v2/get_product_reviews`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-product-reviews 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --sku-id 669ddd44e05f3700011067ed --share-pics-only 1` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/xiaohongshu/app_v2/get_product_reviews` |
...
| `sku_id` | 是 | 见 --help |
| `page` | 否 | 默认 0 |
| `sort_strategy_type` | 否 | 默认 0 |
| `share_pics_only` | 否 | 默认 0 |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/xiaohongshu/app_v2/get_product_reviews` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/420136405e0
