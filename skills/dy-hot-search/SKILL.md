---
name: dy-hot-search
description: 获取抖音热搜榜数据，可按榜单类型筛选。需要一格数据 API Key。触发词：抖音热搜、热榜、dy-hot-search
---

## 简介

抖音热搜榜 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

拉取抖音热搜榜，适合选题、热点追踪与日报。可按 board_type / board_sub_type 切换榜单。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/app/v3/fetch_hot_search_list`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-search-suggest`、`dy-search-video`、`dy-search-general` 组成完整工作流
...
# 将 skills/dy-hot-search 放入你的 Agent Skills 目录
```
...
```bash
export YIGE_API_KEY=ak_xxx
python scripts/fetch.py --board-type 0
```
...
| `python scripts/fetch.py --board-type 0` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/douyin/app/v3/fetch_hot_search_list` |
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `board_type` | 否 | 榜单类型，默认 0 |
| `board_sub_type` | 否 | 榜单子类型，可空 |

### 注意事项
- 热点词可继续用 dy-search-video / dy-search-general
- 榜单结构以返回 JSON 为准
- 按次计费，请关注控制台余额
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/douyin/app/v3/fetch_hot_search_list` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-search-suggest`、`dy-search-video`、`dy-search-general`
