...
def main() -> None:
    p = argparse.ArgumentParser(description="公众号文章列表")
    add_common(p)
    p.add_argument("--username", required=True)
    p.add_argument("--page-size", type=int, default=20)
    p.add_argument("--offset", default="", help="首页留空；翻页传 next_offset")
    p.add_argument("--item-show-type", type=int, default=None, help="0文章/5视频/7音频/8贴图")
    args = p.parse_args()
    ...
    if args.item_show_type is not None:
        body["item_show_type"] = args.item_show_type
    result = post("/v1/wechat_mp/v2/fetch_account_articles", body, api_key)
