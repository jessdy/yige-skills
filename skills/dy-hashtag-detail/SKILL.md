---
name: dy-hashtag-detail
description: 根据 ch_id 获取话题（挑战）详情。需要一格数据 API Key。触发词：抖音话题、抖音挑战、dy-hashtag-detail
---

## 简介

抖音话题详情 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/app/v3/fetch_hashtag_detail`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-hashtag-videos`、`dy-search-general` 组成完整工作流
...
# 将 skills/dy-hashtag-detail 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --ch-id 123456789` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/douyin/app/v3/fetch_hashtag_detail` |
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `ch_id` | 是 | 话题 ID（challenge id） |

### 注意事项
- ch_id 可从话题链接或搜索结果取得
- 继续拉作品用 dy-hashtag-videos
- 按次计费，请关注控制台余额
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/douyin/app/v3/fetch_hashtag_detail` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-hashtag-videos`、`dy-search-general`
