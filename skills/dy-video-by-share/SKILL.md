---
name: dy-video-by-share
description: 根据抖音分享链接/短链解析并获取作品详情。需要一格数据 API Key。触发词：抖音分享链接、抖音短链解析、dy-video-by-share
---

## 简介

抖音分享链接解析作品 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/app/v3/fetch_one_video_by_share_url`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-video-detail`、`dy-video-stats`、`dy-video-comments` 组成完整工作流
...
# 将 skills/dy-video-by-share 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --share-url "https://v.douyin.com/xxxxx/"` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/douyin/app/v3/fetch_one_video_by_share_url` |
...
| 参数 | 必填 | 说明 |
|------|------|------|
| `share_url` | 是 | 分享链接 / 短链（也可是含链接的完整分享文案） |

### 注意事项
- 解析成功后可再取 aweme_id 拉评论或统计
- 偶发短链失效请换原始链接重试
- 按次计费，请关注控制台余额
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/douyin/app/v3/fetch_one_video_by_share_url` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-video-detail`、`dy-video-stats`、`dy-video-comments`
