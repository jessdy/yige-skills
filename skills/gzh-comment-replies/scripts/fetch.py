...
def main() -> None:
    p = argparse.ArgumentParser(description="公众号评论二级回复")
    add_common(p)
    p.add_argument("--url", required=True)
    p.add_argument("--content-id", default="", help="一级评论 content_id，纯数字字符串")
    ...
    if args.content_id:
        body["content_id"] = str(args.content_id)
    result = post("/v1/wechat_mp/v2/fetch_comment_replies", body, api_key)
