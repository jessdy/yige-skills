...
通过 [yige.zone](https://yige.zone/settings/api-keys?source=github) API 解析 YouTube 视频链接，直接返回无水印视频下载链接（资源可能包含不同格式的视频和音频文件）。
...
本工具基于 [yige.zone](https://yige.zone/settings/api-keys?source=github) 的 `parseWork/videoDownload/youtube` 接口构建。前往官网查看更多 API 能力和使用文档。
