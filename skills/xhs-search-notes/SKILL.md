...
description: 按关键词搜索小红书笔记，支持排序、笔记类型与发布时间筛选，翻页需携带 search_id/search_session_id。 需要一格数据 API Key。触发词：小红书搜索笔记、小红书数据、xhs-search-notes
---

## 简介

小红书搜索笔记 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

按关键词搜索小红书笔记，支持排序、笔记类型与发布时间筛选，翻页需携带 search_id/search_session_id。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/app_v2/search_notes`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-search-notes 放入你的 Agent Skills 目录
```
...
```bash
export YIGE_API_KEY=ak_xxx
python scripts/fetch.py --keyword 美食推荐 --sort-type popularity_descending
```
...
| `python scripts/fetch.py --keyword 美食推荐 --sort-type popularity_descending` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/xiaohongshu/app_v2/search_notes` |
| Host | `https://yige.zone` |
| 鉴权 | `Authorization: Bearer <YIGE_API_KEY>` |
| 超时 | 建议 30s |
| 文档 | https://docs.tikhub.io/420136398e0 |
...
| `keyword` | 是 | 见 --help |
| `page` | 否 | 默认 1 |
| `sort_type` | 否 | 默认 "general" |
| `note_type` | 否 | 默认 "不限" |
| `time_filter` | 否 | 默认 "不限" |
| `search_id` | 否 | 见 --help |
| `search_session_id` | 否 | 见 --help |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/xiaohongshu/app_v2/search_notes` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/420136398e0
