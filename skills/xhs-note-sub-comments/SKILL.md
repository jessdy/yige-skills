...
description: 拉取某条一级评论下的二级回复；需 comment_id。 需要一格数据 API Key。触发词：小红书笔记二级评论、小红书数据、xhs-note-sub-comments
---

## 简介

小红书笔记二级评论 基于[一格数据](https://yige.zone) 开放接口，把小红书公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

拉取某条一级评论下的二级回复；需 comment_id。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/xiaohongshu/app_v2/get_note_sub_comments`，鉴权使用 Bearer API Key。
...
# 将 skills/xhs-note-sub-comments 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --note-id 699916e6000000001d0253da --comment-id 699fb9930000000008030db6` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/xiaohongshu/app_v2/get_note_sub_comments` |
| Host | `https://yige.zone` |
| 鉴权 | `Authorization: Bearer <YIGE_API_KEY>` |
| 超时 | 建议 30s |
| 文档 | https://docs.tikhub.io/420748830e0 |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/xiaohongshu/app_v2/get_note_sub_comments` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 开放文档：https://docs.tikhub.io/420748830e0
