---
name: dy-user-profile
description: 根据 sec_user_id 获取抖音用户公开资料（粉丝、关注、简介等）。需要一格数据 API Key。触发词：抖音用户信息、抖音主页、dy-user-profile
---

## 简介

抖音用户资料 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

按 sec_user_id 拉取用户公开资料。抖音用户主键多为 sec_user_id，可从作品详情或用户搜索结果取得。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/app/v3/handler_user_profile`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-search-user`、`dy-user-posts`、`dy-user-fans` 组成完整工作流
...
# 将 skills/dy-user-profile 放入你的 Agent Skills 目录
```
...
```bash
export YIGE_API_KEY=ak_xxx
python scripts/fetch.py --sec-user-id MS4wLjABAAAA...
```
...
| `python scripts/fetch.py --sec-user-id MS4wLjABAAAA...` | 常用示例 |
...
|------|------|
| Method | `GET` |
| Path | `/v1/douyin/app/v3/handler_user_profile` |
...
### 注意事项
- 优先用 sec_user_id，不要只依赖 nickname
- 可继续拉主页作品/粉丝列表
- 按次计费，请关注控制台余额
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `GET /v1/douyin/app/v3/handler_user_profile` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-search-user`、`dy-user-posts`、`dy-user-fans`
