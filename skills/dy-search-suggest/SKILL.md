---
name: dy-search-suggest
description: 根据关键词获取抖音搜索联想/推荐词。需要一格数据 API Key。触发词：抖音联想词、搜索建议、dy-search-suggest
---

## 简介

抖音搜索联想词 基于[一格数据](https://yige.zone) 开放接口，把抖音公开数据以结构化 JSON 返回，方便 Agent 与业务系统直接消费。

输入关键词获取搜索联想，适合扩词、选题与 SEO 词包建设。
...
技术基础：Python 3 标准库（`urllib` / `json`），调用 `https://yige.zone/v1/douyin/search/fetch_search_suggest`，鉴权使用 Bearer API Key。
...
- **🔗 可编排**：可与 `dy-hot-search`、`dy-search-video`、`dy-search-general` 组成完整工作流
...
# 将 skills/dy-search-suggest 放入你的 Agent Skills 目录
```
...
| `python scripts/fetch.py --keyword 人工智能` | 常用示例 |
...
|------|------|
| Method | `POST` |
| Path | `/v1/douyin/search/fetch_search_suggest` |
...
| Python 3 | 运行环境 |
| urllib | HTTP 请求 |
| 一格数据 API | `POST /v1/douyin/search/fetch_search_suggest` |

### 资源索引
- 脚本：`scripts/fetch.py`
- 接口文档：https://yige.zone/docs
- 控制台密钥：https://yige.zone/settings/api-keys
- 相关 Skill：`dy-hot-search`、`dy-search-video`、`dy-search-general`
